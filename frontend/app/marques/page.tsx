          ...
          <table className="w-full caption-bottom text-sm">
            <thead className="[&_tr]:border-b">
              <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                ...
                {isAdmin && <th className="h-10 px-4 text-right align-middle font-medium text-muted-foreground">Actions</th>}
              </tr>
            </thead>
            <tbody className="[&_tr:last-child]:border-0">
              {loading ? (
                <tr><td colSpan={isAdmin ? 4 : 3} className="p-4 text-center">Chargement...</td></tr>
              ) : marques.length === 0 ? (
                <tr><td colSpan={isAdmin ? 4 : 3} className="p-4 text-center">Aucune marque trouvée.</td></tr>
              ) : (
                marques.map((item) => (
