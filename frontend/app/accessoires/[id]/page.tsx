...
import { avisService, AvisDTO } from '@/lib/api/avisService';
import { useCart } from '@/lib/context/CartContext';
import { useWishlist } from '@/lib/context/WishlistContext';
import { Star, Truck, ShieldCheck, Check, ShoppingCart, Edit, TrendingUp, Heart } from 'lucide-react';
...
export default function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  const [reviews, setReviews] = useState<AvisDTO[]>([]);
  ...
  const fetchProductAndReviews = useCallback(async () => {
    try {
      setLoading(true);
      const [resProd, resAvis] = await Promise.all([
        accessoireService.getById(parseInt(params.id)),
        avisService.getByAccessoireId(parseInt(params.id))
      ]);
      setProduct(resProd.data);
      setReviews(resAvis.data || []);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  }, [params.id, fetchProductAndReviews]);
  ...
  const handleApplyPromotion = async () => {
    const nouveauPrixStr = prompt("Entrez le nouveau prix promotionnel (TND) :");
    if (nouveauPrixStr && !isNaN(parseFloat(nouveauPrixStr))) {
      try {
        await accessoireService.applyPromotion(product!.id, parseFloat(nouveauPrixStr));
  ...
  const handleAddReview = async (e: React.FormEvent) => {
    ...
    if (newReview.comment.trim() && product) {
      try {
        ...
        // Refresh reviews
        const resAvis = await avisService.getByAccessoireId(product.id);
        setReviews(resAvis.data || []);
      } catch (e: any) {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 animate-in fade-in duration-500 pt-8">
      ...
      <div className="grid md:grid-cols-2 gap-12">
        <div className="bg-slate-50 dark:bg-slate-900 rounded-3xl p-8 flex items-center justify-center aspect-square border relative">
          ...
          {isAdmin && (
            <div className="absolute top-4 right-4 flex gap-2">
              ...
              <Link 
                href={`/accessoires/${product.id}/edit`} 
                className="bg-white dark:bg-slate-800 text-slate-700 dark:text-white p-3 rounded-full shadow-lg hover:text-primary transition-colors"
          ...
          <div className="pt-6 border-t dark:border-slate-800 space-y-4">
            <div className="flex items-center gap-4">
               ...
               <div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg p-1 w-32">
                 ...
                 <button onClick={() => setQuantity(Math.min(product.stock, quantity + 1))} className="p-2 hover:bg-white dark:hover:bg-slate-700 rounded-md transition-colors flex-1 font-bold">+</button>
               </div>
            ...
            <div className="flex gap-4">
              <button 
                ...
                className={`flex-1 py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 transition-all ${added ? 'bg-green-500 text-white' : 'bg-primary text-white hover:bg-primary/90'} disabled:opacity-50`}
              >
              ...
              <button 
                ...
                className={cn(
                  "px-6 rounded-xl border-2 transition-all flex items-center justify-center gap-2 font-bold",
                  isInWishlist(product.id) 
                    ? "border-red-500 bg-red-50 text-red-500" 
                    : "border-slate-200 hover:border-red-500 hover:bg-red-50 hover:text-red-500 text-slate-600"
                )}
              >
      ...
      <div className="grid md:grid-cols-3 gap-12 pt-12 border-t dark:border-slate-800">
         ...
         <div className="space-y-8">
           ...
           <div className="space-y-4">
              {reviews.length === 0 ? (
                <div className="text-slate-500 italic">Aucun avis pour l'instant.</div>
              ) : (
                reviews.map((rev) => (
                  <div key={rev.id} className="bg-white dark:bg-slate-900 p-5 rounded-2xl border shadow-sm space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="font-bold">{rev.nomUtilisateur}</span>
                      <div className="flex text-yellow-400">
                        {Array.from({length: 5}).map((_, i) => <Star key={i} className={`h-4 w-4 ${i < rev.note ? 'fill-current' : 'text-slate-200 dark:text-slate-700'}`} />)}
                      </div>
                    </div>
                    <p className="text-slate-600 dark:text-slate-400 text-sm">{rev.commentaire}</p>
           ...
           <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-2xl border">
             ...
             {!isAuthenticated ? (
               ...
               <form onSubmit={handleAddReview} className="space-y-4">
                 ...
                 <button type="submit" disabled={submittingReview} className="w-full bg-slate-900 dark:bg-primary text-white font-bold py-2 rounded-lg hover:bg-slate-800 dark:hover:bg-primary/90 transition-colors disabled:opacity-50">
